...
### Ministry F hard-gate and broad-need correction — 2026-09-26

| Requirement | Local evidence | Disposition |
|---|---|---|
| Core 04/07 hard-gate classifier and summary dedupe; Core 02/09 proof | ADR-032 proposed. Focused pytest on capability, scenario, graph and all-eleven outcome files. Evidence outside the candidate: mission `ministry-foundation/evidence/`. | IMPLEMENTED in source. Verification for the new candidate pending. Not accepted. |
| Broad evidence needs target Product | Graph projection, feed, service and API tests; in-memory semantic comparison reserved for the source-readiness note. | IMPLEMENTED in source. Generated graph not written. |
| Unchanged decisions, economics and isolation | Existing outcome, golden, public-decision, fidelity and synthetic-isolation assertions in the same local run. | Exercised locally. Full `make ci`, manifests and visual pins remain pending. |
