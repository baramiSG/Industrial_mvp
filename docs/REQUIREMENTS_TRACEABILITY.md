...
| FR-002 | R-rule thresholds loaded from versioned YAML | Core 01 §6.1 | `config.thresholds_config`, `rules.py`; literals remain in `decision_engine`, `rules` R11 | `/api/thresholds` | rule ledger caption | `test_rules.test_threshold_is_loaded_from_versioned_config` | AST literal scan **missing** | IMPLEMENTED (partial) | S02 |
...
| FR-025 | R11 supports rejection of generic capacity | Core 01 §6.3 | `rules.py` R11 (literal `50`) | yes | yes | `test_pp_generic_capacity_warning_fires` | — | IMPLEMENTED (partial) | S02 |
...
| FR-033 | D\* not published below Kmin or with unresolved hard gate | Core 01 §6.4 | `capability.evaluate_capability` | — | "GATED" | `test_public_steel_dstar_is_withheld_for_unresolved_hard_gates` | boundary tests missing | IMPLEMENTED | S02 |
| FR-034 | Route bands (4) | Core 01 §6.4 | `capability.route_band` | — | matrix | golden | band boundary tests missing | IMPLEMENTED | S02 |
...
| FR-044 | Post-entry capacity/downside ratio visible | Core 01 §6.5 | `decision_engine._simulate_steel` (literal `1.25`) | `competition` | panel | golden `< 1.25` | boundary missing | IMPLEMENTED (partial) | S02 |
...
| GATE-C | Rules visible; thresholds from config | Core 09 §6 | partial (literals) | IMPLEMENTED (partial) | S02 |
