...
| 15 — Decision Dossier | One-page decision backed by the complete structured evidence pack; public conclusion preserved in simulation | `dossier.py`, `dossier_projection.py`, `dossier_validation.py` | dossier projection, isolation, availability, API and rendered PDF tests | bilingual JSON/HTML and native Chromium Print / Save PDF |
...
| Interactive graph GenUI descriptor | `genui.build_ui_manifest`, `static/modules/renderers/index.js` | Methodology §8.3; Core 03 §§7, 14 | `test_genui_decision_contract.py`, `test_s17_graph_contract.py` | collapsed `graph_view` after decision actions |
