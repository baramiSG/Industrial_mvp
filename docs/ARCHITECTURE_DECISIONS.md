...
## ADR-026 — S16b shared-enabler valuation, route-eight activation and mounted graph API

**Recorded T1/T2 status:** uncommitted implementation candidate under approved child plan `b8ba2321…`, execution-method amendment `S16B-EXEC-HISTORY-01` and the owner-authored interface/query shapes.

**Current status:** exact local and portability product verification completed at `c0d5ec2` with 2,854 unit tests, 498 functional and four visual checks in each root, plus local live/unavailable graph proof. The original graph, manifest and visual allocations were each consumed once. Independent implementation review and owner acceptance are complete. The mandatory Aura proof passed on `8a7338e0` with no clear operation. PR/hosted CI and delivery remain pending at this capture point.

**Authority and preservation.** Methodology §§7.4, 8.2–8.3 defines the lower-route sequence and UnlockValue. Scenario contract 2.1.0 is additive beside 2.0.0. The owner retained exact pre-edit histories for thresholds 1.2.0, decision narratives 1.3.0 and the two aluminium scenarios at 2.0.0 before any live-source edit. Public snapshots, original goldens, the other nine scenarios, prior graph projections, visual roots, installed controls and unrelated work remain frozen.

**Declaration and valuation.** A complete `shared_enabler` block is legal only in 2.1.0. It carries exact common and dependent fields, six boolean components, competition inputs, a strict non-boolean valuation route code 1–7 and Class-D design basis. The valuation code resolves exactly one declared route record; route 5 uses its existing economics inheritance and other routes use their own national-value block. Ground truth and derived decisions never select or supply valuation. Complete-set validation rejects duplicate identities/membership and common-field conflict. Gate B appends `shared_enabler_declaration_valid` as check 11.

The governed pair uses `ENABLER-SYN-ALU-CASTHOUSE-001`, cost 25 M SAR, foil P/share 0.6/0.5 against route 6 value 172 and profiles P/share 0.7/0.4 against route 4 value 31. The result is 35.28 M SAR. Both route records remain fully resolving, so route 8 is evaluated then fails `LOWER_ROUTE_FULLY_RESOLVES`; selected routes 6/4 and every public decision remain unchanged.

**Projection and engine boundary.** Phase one validates and projects non-derived `UNLOCKED_BY` evidence with valuation code/reference and exact Scenario-to-Product membership. Before derived outputs, branch-qualified artifact feeds are injected into the public and simulated low-level functions. No low-level function loads the current artifact, invokes the builder or opens Neo4j. Application wrappers use the cached validated current artifact and fail closed on missing, corrupt, mismatched or declaration-less inputs. An honest non-persisted fixture with probabilities/shares 1/1 computes UnlockValue 178, selects ADVANCE/8 and passes the real ground-truth back-test; restoring a lower fully resolving route blocks escalation.

**Route and API behavior.** The exact `None` route-8 record remains `GRAPH_REQUIRED` without evaluated-only fields. Evaluated route 8 validates aligned finite inputs, configured minimum dependents, positive UnlockValue, component, additionality, policy and competition gates; it participates in lower-route precedence and maximum unrounded value selection with the existing lower-code tie rule. The existing graph router is mounted after screening and before the SPA fallback. Canonical artifact corruption maps to sanitized 422; post-status driver/connection failures map to typed `GRAPH_UNAVAILABLE`; offline-guard and unexpected exceptions propagate. The owner-authored shared-enabler Cypher query is retained exactly.

**Configuration and original T1/T2 generation hold.** Thresholds advance 1.2.0→1.3.0 only for `routes.shared_enabler.minimum_dependent_opportunities = 2`; decision narratives advance 1.3.0→1.4.0 only for the two route-8 failure templates. Graph pycache output is parameterized while retaining the old default. Persisted graph/manifests were deliberately held during T1/T2. The subsequent original allocations each ran once: graph `GRAPH-SAU-2026-09-12-fbc93afb3c6e` at 925/1,045, snapshot/authority inventories 719/20 and 96 visual images. Exact reconstruction, integrity and complete product verification then passed; no test-only correction repeated those generators.
...
**Decision:** Append one `graph_view` descriptor after `decision_actions` while retaining manifest version 1.0 and every prior descriptor. The panel is collapsed and request-free initially. Its state binds an independent epoch to opportunity, evidence mode and fixed view; context changes and closure clear graph, selection and evidence synchronously, and both successful and failed responses must match the current context before any state or cache update.

The frontend validates unique governed node/edge identities, endpoints, context, provenance and public/Class-D partition before rendering. Deterministic geometry mirrors coordinates for RTL without reversing source/target semantics. SVG pointer interaction and equivalent native HTML buttons share selection behavior; the SVG adds no duplicate tab sequence. Explicit bilingual maps cover all 19 node labels, 15 edge types, four explanations and supported failures.

Evidence drill-down unions provenance, property and drill-down references and resolves them only against stored evidence in the current analysis or existing analysis endpoints for visible portfolio products. Same evidence IDs in different opportunities remain qualified records. Missing references and document addresses remain explicit, arbitrary markup is escaped, and external links accept only HTTP or HTTPS.

UI strings advance 1.4.0→1.5.0 under Manifest §7.3 with effective date 2026-09-16. The exact 1.4.0 bytes are retained under `config/history/`. All prior keys and values remain unchanged; synthetic warning labels remain exclusively policy-owned. Core 02/03/09 changes are the S17 implementation mapping under ADR-010 and the approved plan.

**Dependency correction:** `S17-DEP-AM1` permits the smallest graph-service isolation fix after actual serialization exposed two Class-D evidence-support edges in a public payload. Only matching public/PUBLIC edges, plus exact-current-scenario synthetic edges in simulated mode, may be appended for evidence drill-down. Top-level synthetic state and bilingual labels derive from returned nodes and edges. Shared-enabler cross-scenario dependency edges remain unchanged. Regressions cover all eleven opportunities and all four public views, edge-only synthetic warnings and both governed aluminium dependency edges.
...
## ADR-029 — Bounded executive contract hardening and immutable S18a delivery evidence

Status: plan accepted under the owner overnight delegation after independent same-model Codex plan APPROVE; focused implementation verified and exact-candidate gates pending. This is a new follow-up decision, not a modification of ADR-028's historical S18a packet or its Claude approval.

Context: AM4 left precise malformed-input and model-partition observations for follow-up. Read-only probes reproduced bare errors at empty trade/latest-year/required R11 boundaries and coercion of supplied EVSI values, plus model acceptance of a public flag with synthetic status or DEMO_GENERATOR source. Normal validated canonical inputs remain valid; production reachability is not asserted from these injections alone.

Decision: validate at those immediate executive projection boundaries and raise the existing ExecutiveIntegrityError. Preserve the API exception mapping; do not catch arbitrary programming errors globally. Supplied EVSI numerics must be finite numeric inputs excluding booleans and strings; optional absence remains allowed and zero remains calculable. Public ClassD proxies remain legitimate; reject synthetic provenance markers rather than classD alone. Dependent scenario mappings must be checked without expanding foreign-evidence exemptions. Valid response/schema/decision bytes remain unchanged. New helpers belong only inside the executive package and no frozen/config/Core/graph/visual generation is authorized.

Verification: test each reproduced defect against original source first, retain meaningful positive controls and AM4 assertions, compare all valid responses/analyses, then freeze the full source+documentation candidate for prescribed clean-root gates and independent review. Exact candidate identities and gate outcomes belong in external receipts; the candidate does not embed its own tree ID. No author approves its own work.

Delivery evidence: S18a was owner-accepted and Claude-approved at tree1364789a, then delivered by PR37/mergeb4a00ad with all PR/main checks green. Publish the approved immutable54 evidence records plus publication manifest and new wrapper/checksums/delivery record, preserving the original receipt SHA256`8fc1368371a067bc3e2fe99359600be5d01c663024644e9e3d8147478e4122fb`. The complete856-file bundle remains preserved; the subset is not presented as the full bundle. No archive/index/credential/transcript enters the export.

[Approved plan](../.workflow/slices/S18b0-executive-contract-hardening/plan.md), [independent review](../.workflow/slices/S18b0-executive-contract-hardening/plan_review.md) and [S18a delivery](../.workflow/slices/S18-executive-mode/delivery_record.md) are the evidence chain. Explicit case-level EVSI schema availability, broad R12 case-wide needs and untested malformed paths remain open. Same-model overnight review does not satisfy the final independent model-family release gate; no release tag is authorized by this ADR.

Focused evidence: the original new-regression run failed113 tests and passed47 before repairs; the complete repaired focused suite passed290 tests. All12 raw API responses,12 canonical responses and22 full analyses match the accepted base exactly. An implementation-introduced import cycle was independently reviewed and corrected using strict finite Pydantic fields with unchanged schema; the failed log and correction approval are retained. See [implementation log](../.workflow/slices/S18b0-executive-contract-hardening/implementation_log.md) and [test evidence](../.workflow/slices/S18b0-executive-contract-hardening/test_evidence.md). These focused results do not substitute for the prescribed full gates or independent approval.

Independent R1 review subsequently found oversized-integer conversion and finite-input aggregate overflow. The bounded correction preserves successful sums and translates only those failures into the same sanitized integrity error. Its regression run on the rejected production code failed28 tests and passed315; the corrected focused suite passed343 with all46 reference artifacts unchanged. See the [finding record](../.workflow/slices/S18b0-executive-contract-hardening/reviewer_findings.md). R1 rejection, interrupted full gates, correction review and delegated acceptance remain preserved. Full gates and independent review must run on the new exact candidate.

## ADR-030 — Governed bilingual executive surface and ordered provenance closure

Status: S18b plan and AM1 independently approved under the owner overnight delegation; implementation and exact-candidate verification remain in progress. Same-model Codex agents provide tonight's separate reviews; this is not Claude approval or final release acceptance.

The accepted [plan](../.workflow/slices/S18b-bilingual-executive-surface/plan.md), [experience design](../.workflow/slices/S18b-bilingual-executive-surface/design.md) and [review record](../.workflow/slices/S18b-bilingual-executive-surface/plan_review.md) authorize A1–A5: selected-case presentation joins; retention of112 existing visual paths plus16 executive captures with reviewed declared differences; ordered graph/manifest/visual provenance closure; correction of the live KL-132 sentence/test; explicit AM4 residual tracking. No domain formula, threshold, route order, evidence class, frozen snapshot/scenario/golden or public outcome changes are authorized.

Core02 adds the methodology mapping, Core03 the fixed eight-step registry/data-flow boundary, Core09 the functional and128-image acceptance contract, and ui_strings1.6.0 the exact bilingual chrome. These are governed UI/implementation changes under Manifest§7.2 and existing Core authority. AM1 permits only the two validator literals needed to accept that catalogue version; no broader config validation change is included. Authority/config changes are declared here before any manifest generator runs. The original methodology DOCX and all operating thresholds/evidence-policy bytes remain unchanged.

The frontend consumes server claims/counts/EVSI and existing selected-case narrative/economics detail, validates complete public decision equality and branch provenance, and renders typed unavailable values separately from numeric zero. Class-D results carry both policy labels and actual scenario identity. Native details, links and keyboard controls preserve the analyst workspace. Source text remains verbatim and explicitly marked rather than translated or presented as a UI label.

Generation is a separate coordinator operation after writer pause and hashed-input finalization: bind graph source identity, create a new write-once projection, verify unchanged925/1045 semantics and all eleven decisions (including both public goldens), update snapshot/authority manifests, then allocate exactly one reviewed canonical visual refresh. Existing artifacts remain immutable. Unexpected output or unrelated failures require reviewed correction; hashes are not automatically accepted. Exact candidate/receipt/CI/review identities belong in additive external delivery records, avoiding a self-referential tree hash.

B0 was delivered before this writer's base at merge306a63b4 (PR38, six main jobs passed per coordinator delivery evidence); its bounded malformed-input/partition fixes are not reimplemented here. Case-level EVSI schema availability, unenumerated malformed paths, R12 case-wide attribution, remaining validator coverage and routed analyst-shell limitations remain explicitly tracked. Live Aura and S19–S22 remain separate units. No source author approves its own candidate.

ADR030 AM2 addition: the reviewed graph-name/containment correction changes only presentation and its exact UI scroll-hint pair. Core02/03/09 and that existing catalogue input must be final before graph generation. Static graph code and browser helpers are visual inputs; they do not introduce new graph discovery inputs. The same graph925/1045, snapshot726/authority20 and128-image600KiB/16MiB contracts remain required. No generation has been run by the implementer.
...
### ADR-030 additive AM4 quality closure
...
The separately reviewed endpoint oracle correction and lowercase capability-key addendum are preserved with attribution in the slice plan-review record. Original visible U and all authority semantics remain; no global key grammar or browser parity relaxation is admitted.

### ADR-030 additive AM5 and parent-scope presentation corrections

[AM5](../.workflow/slices/S18b-bilingual-executive-surface/plan-amendment-5.md) adds exactly six governed integrity labels and focused Core03§7.1/Core09§12 requirements before manifest generation. It displays actual computed checks/counts, rejects conflicting analyst response identity, and localizes the existing unconfirmed screening reviewer status. These Manifest§7.2 presentation/contract changes leave methodology, thresholds, snapshots, source classes and industrial decisions unchanged. Final public golden and integrity proof remains part of the ordered generation/exact-candidate gates.

Separate parent-scope corrections expose actual vector claims, join genuine no-scenario absence without inventing an EVSI row, preserve readable vector words, and position the16 executive scenes over complete semantic content. Two literal public unavailable-step claim IDs use the existing narrow technical grammar; no broad Latin or source exception is introduced. All112 prior capture preparations remain unchanged. These corrections have independent scope reviews and delegated acceptance recorded in the slice plan-review history, not within the immutable AM5 packet. Source/UI approval, resource acceptance, provenance generation and final delivery remain pending.

### ADR-030 additive execution and finding disposition — 2026-09-24

Actual Claude Code approved OPS R2's finite replacement execution approach; separate Codex reviews approve their exact source/operation subjects. Retired prediction, startup/module-origin, process-tracking, CI-guard and adapter sources/failures remain preserved. Existing generators ran once each; the canonical128 operation and both encoding supplements are independently approved and accepted without product changes or recapture. Final acceptance still requires R1, two full clean-root CI runs and exact reviewed delivery. [The slice record](../.workflow/slices/S18b-bilingual-executive-surface/pr_record.md) carries actual identities; no future candidate embeds its own tree OID.

AR-V01 applies the existing S18b unchanged-dossier boundary and S19 responsive/no-clipping scope: inherited pixels are preserved but clipped regions receive no quality approval. Original-defect RED→GREEN for both actual Arabic1024 simulated dossiers, complete identity/both warnings and EN/AR public/simulated responsive regressions are mandatory before S19/milestone completion. No domain/schema/threshold change or final Aura certification is introduced.

### ADR-030 additive genuine-defect correction — 2026-09-24

The reviewed amendment corrects the demonstrated defects without changing graph/domain behavior or acceptance thresholds: `.graph-element-button[data-graph-select="edge"] > small` gains `flex-grow:1`; the existing graph browser test preserves meaningful original-ID EN/AR RED→GREEN coverage; two exact manifest assertions become726. No R1 comparator, anchor, tolerance, route, fixture generator or graph input changed. Graph/manifests are reused through actual byte/hash preservation; changed visual inputs received the separately accepted canonical R2 refresh and still require a new exact candidate. Previous approvals/failures remain historical; actual Claude review of the previous CI binding does not transfer to the next candidate. [Exact records](../.workflow/slices/S18b-bilingual-executive-surface/pr_record.md).

## ADR-031 — Structured bilingual dossier projection and native PDF export

Status: approved S19 plan/amendments accepted under the owner delegation; implementation and its independent acceptance remain pending. This records the authorized authority-input change **before** manifest generation. Parent plan SHA-256 `7749006ae850adf66fea862934c8362d9ea5c9254aaf74ded702504e69ce44d1` and separate R2 review `9da12809d4313754169bb4955d32063a4cbacc18eefebf3ca7ddf9c4a633e88e` are retained [byte-for-byte in S19](../.workflow/slices/S19-bilingual-dossier/accepted-packets/00-s19-S19-IMPLEMENTATION-PLAN.md). Separate successor-pin, AR-V01, base-adoption, TRADE-SCALE-01 R2 and renderer-placement packets retain their historical wording and independent attribution. The actual dependency is delivered S18b merge `ce407db9832b61c5a8a85dfda2e3b7623da2fbc9` from PR39 with six required main checks successful.

**Decision.** Extend the existing JSON→HTML export to common DecisionDossier2.0.0, preserving old keys/meaning and adding detached `public_decision`, structured methodology `blocks` and `evidence_pack`. Public never loads scenario context; simulation validates exact identity/Class-D/policy markers and a finite field whitelist. Keep complete decisions/routes/passports, real null/zero distinctions and exact source paths; missing fields have null paths. Rendering remains in existing `dossier.py` with actual consumed catalogue mappings, while `dossier_projection.py` and `dossier_validation.py` separate projection/validation. The two initially drafted rendering helper modules are consolidated and their preimages retained externally. Stock copy/usage checks are unchanged.

**Exact authority-input boundary.** Change only `docs/core/02_METHODOLOGY_IMPLEMENTATION_MAP.md` (mapping), `03_SYSTEM_ARCHITECTURE.md` (export/shared-chart responsibilities), `04_CANONICAL_DATA_MODEL.md` (additive2.0.0 projection), `09_TEST_ACCEPTANCE_AND_GOLDEN_CASES.md` (acceptance) and `config/ui_strings.v1.yaml` (1.6.0→1.7.0, effective2026-09-24, dossier labels and six paired trade labels). These are Manifest§7.2 implementation/presentation changes under the existing methodology; no methodology DOCX, threshold, sector-profile, evidence-policy, public snapshot, scenario or golden expectation changes. Steel public remains INVESTIGATE and polypropylene public remains REJECT for generic capacity support. Complete all11 real decisions must remain equal, beyond just state/route.

**Other file permissions.** The parent's exact file set and the finite accepted amendment tables are the exhaustive boundary: dossier façade/projection/validation and shared API error wrapper; scoped dossier/print tokens, new `dossier-actions.js`; existing shared trade renderer and `.trade-chart` styles; named dossier/catalogue/static/browser tests, actual reviewed inventory additions and the single native-summary keyboard expectation; named control/runbook/UX docs and new S19 slice records. Base adoption advances only two live graph-token literals and two live manifest-count literals after actual output binding. Successor adoption changes only exact live catalogue/disclosure/adapter/module/browser literals. No script, checker, Makefile, CI, executive product module, dependency or historical fixture edit is admitted.

**Experience.** Existing Executive→Analyst navigation preserves case/locale; the user explicitly selects analyst evidence mode before export. A screen toolbar provides native Chromium Print / Save PDF, JSON and validated return navigation. No server PDF endpoint or product browser dependency. The first A4 page is a readable summary, followed by all eleven appendix sections and full conditions/kill conditions. Both warnings repeat on every simulated page. The trade repair retains existing curves/units and adds the approved independent-scale explanation and native observed-data table; it adds no analysis or new metric. AR-V01 and TRADE-SCALE-01 remain open pending their actual closure proof.

**Generation and acceptance.** Finalize these authority/code/capture inputs and source/UI review first, then perform the separately bound read-only double build/full graph and complete all11 decision comparison. Authorize one write-once graph projection and one manifest build with every old root preserved; actual expected topology remains925/1045. Snapshot726→728 is conditional on exactly two added graph paths, never a forced count. Then bind one canonical128-image operation:16 dossier redesigns plus only the trade amendment's eligible56 chart-consumer paths, with unchanged dimensions/fonts/producer/tolerances and600KiB/16MiB budgets. Strict R1, actual baseline subtree pin, all44 PDFs/every-page inspection, two isolated full clean-root CIs, independent exact-tree/head review, hosted/main checks and delegated acceptance remain mandatory. Exact candidate IDs and post-merge facts belong in additive external/delivery records, avoiding self-referential tree content.

**Limitations.** Missing evidence stays missing; no official Ministry evidence, PDF-UA certification, source-author approval or Claude approval is asserted. Same-model independent Codex review is temporary under the owner mandate. AM4 API/model/test residuals, final S21 live Aura/application verification and S22 model-family certification remain separate open obligations. This ADR authorizes neither generation replay nor acceptance of unfinished product work.


### ADR-031 additive D-S19-1 completion correction — 2026-09-25

The owner’s new operational mandate prospectively replaces the09:14:50 S19 stop and prior delivery-then-stop/manual-seat restrictions; their records remain historical. Codex is delegated coordinator/planner/recorder, Cursor Grok4.7 High the primary implementer, GPT-6 Sol High the correction/fallback implementer, and Claude Code the independent reviewer. The actual Project-Build-Guide.docx SHA256`f790d20335a6435b4e1b6e39f0c24f1c776f1938ad483e0a9df7d80255d3ed3e` plus the mandate supersede the missing external workflow dependency for this mission. No methodology/security/evidence gate is waived. Only authenticated subscriptions are authorized; no API-key or paid fallback.

Claude approved the completion addendum51cd644c… in review SHA256`9b83fef253e0854ba1d15d2938dc9711f7a7dc2cac103c65ac7e8297cff332b3` and required D-S19-1: split page-one supporting references and public contradictions using existing bilingual catalogue keys. The two-paragraph renderer correction and focused tests preserve all data, decisions, calculations, font sizes, CSS, catalogue1.7.0 and appendix contents. Actual Claude CLI source-readiness APPROVE is retained at review SHA256`7e78bf3ba431f148f1bdadeb94582c01b4eca4bd87cd0c3c7e55ff417206e867`; reviewed source ledger`5688e75de20572104d0464f77b65ac52f0594d592ff50bf046fbd7bb9298ad08`, renderer`34f20faa89ea15f441285e56b238702df9de960097b4da69ed01336f4c3fb0f0`, test`af95c0e13822ccb32d9da7e1fc5d7112443bf1a9828786ad87801ffcd2f30915`. This is source readiness, not implementation acceptance.

This renderer is a graph and visual input, so the prior ccd1 projection/capture remains accepted historical evidence but cannot certify the corrected candidate. One prospectively allocated replacement generation/capture sequence needs its own exact binding and independent GO. Read-only construction predicts GRAPH-SAU-2026-09-12-6f43b1a8c4aa / ENGINE-6b54371e99f3 with925/1045 topology and230inputs; these identities must match emitted outputs, never be forced. The graph-input delta from ccd1 is only dossier.py. All11 complete real decisions and normalized graph semantics compare equal to delivered ce407db. No projection has yet been written under this correction.

Before that manifest operation, record the existing admitted literal boundary precisely: after actual output corroboration, browser_tests/test_graph.py changes only its new_token to the emitted engine token; old_token and the original-CSS sensitivity rules remain governed by the accepted base-adoption amendment. After manifest membership is verified as old728 plus exactly two new graph paths, the live counts in tests/test_integrity_contract.py, tests/test_s17_generation.py and tests/test_s17_status_docs.py may become730. This explicitly includes the parent-existing status-doc permission, not an additional test permission. Later only the live change_ref and frozen baseline subtree pin change after accepted capture. Preserve historical S17 assertions and every frozen evidence/golden pin. No other test weakening or source edit is admitted.

All44PDF strict/manual proof remains required, although Claude’s new in-memory differential found only four steel HTML first pages change (superseding the earlier shared-template44-change assumption). Actual-predecessor R1 uses delivered ce407db, not ccd1. Final source/visual bindings, full gates, compare-only e2e, two clean-root CIs with their own44PDF proofs, Claude exact-tree/head review, separate delegated acceptance, GitHub/main CI and separately reviewed live Aura work remain pending. The Aura target is instance/database8a7338e0, display name Industrial_mvp; no blanket clear is authorized. No final tree ID is embedded here.

### ADR-031 additive replacement execution checkpoint — 2026-09-25

The preceding D-S19-1 prospective paragraph is retained as history. The existing-key, two-label renderer correction changes no catalogue version, calculation, threshold, source class, scenario or public decision. Because `dossier.py` is in the graph and visual source closures, the corrected bytes required their own replacement generation and capture. The accepted ccd1/r4 subjects do not certify that source.

The single replacement allocation executed: `GRAPH-SAU-2026-09-12-6f43b1a8c4aa`, `ENGINE-6b54371e99f3`, 925/1,045, 230 inputs, snapshot 730, authority 20, five immediate gates. Claude capture-result APPROVE `c32c3425681e21d5383ceabc180c03b5698db108645114b660754d219663d9ee` accepts manifest `0d05eee71eb214c65d21209b52d3933144bb888eb1ddc100f9006e9c9d3621bc`. This unit adopted only the two live successor pins and the finite record assembly. Those pin files are outside the 109 visual and 230 graph inputs. No final tree identity is embedded here.

## ADR-032 — Hard-gate status and broad evidence-need targets

Status: Plan accepted under delegated P1; implementation verification and independent implementation approval pending. This records the F contract correction only. Company discovery (M) is not executed. No final tree identity is embedded here.

**Decision.** Apply the ADR-010/011 Manifest §7.4 Core 04/07 contract-correction precedent. The governing methodology DOCX is unchanged. One case-folded prefix classifier maps `resolved`, `known failure`/`known_failure`, and `not applicable`/`not_applicable`; every other declaration is `UNAVAILABLE`. The stored enums are `RESOLVED`, `KNOWN_FAILURE`, `NOT_APPLICABLE` and `UNAVAILABLE`. Raw declarations stay in their records. Typed `NOT_APPLICABLE` keeps that meaning and is not relabeled `RESOLVED`. A known failure is not an unknown and blocks D* publication. Summary unresolved and known-failure identifiers are unique ordered unions; profile and decision-specific source lists both remain. Broad evidence needs target the Product. A specification or route target is used only when that node already exists in the same scenario and synthetic partition.

Methodology §6.4 states 0/1/2/3/U, λ, Kmin, route order and real-decision meaning are unchanged. Public snapshot and case-brief validators still reject `NOT_APPLICABLE`. Steel public remains `INVESTIGATE`. Polypropylene public remains `REJECT` for generic capacity support. Diagnostic count and graph-endpoint differences are enumerated observable corrections, not a behavior-preserving refactor.

**Authority inputs.** Core 02 mapping and proof clarification, Core 04 hard-gate and broad-need contract, Core 07 publication clarification, Core 09 Gate D proof clarification, this ADR, and the five control documents. No threshold, sector-profile, evidence-policy, scenario, public, synthetic or golden input changes.

**Not in this decision.** Manifest generation, graph write, visual pins, Aura, and the M methodology, register, discovery and disclosure work. Claude's plan approval is not implementation approval.

**F generation and capture allocation — 2026-09-26.** The coordinator's delegated acceptance of the reviewed v3.1 F scope and exact corrected source releases one stock local graph generation, one manifest build, then one canonical visual capture after the generated subjects and admitted literal updates are verified. The source corrections preserve missing required gates as unknown and keep profile/decision declarations independently attributed. The source-bound operation receipt is `mission-20260925/ministry-foundation/GENERATION-RELEASE.json` in existing local recovery records. It supersedes only the preceding source-phase exclusion, not any historical artifact. Actual output IDs, membership counts and hashes are derived and checked, never forced. The approved live-count, engine-token, exact endpoint, passport-span, visual-manifest and baseline-subtree pin updates are admitted only against their emitted subjects. Public/synthetic/golden/entity/screening roots, configuration and prior projections remain unchanged. This allocation authorizes no Aura operation; independent final implementation review and separate delegated implementation acceptance remain pending. Latest owner procedure reuses unchanged receipts and removes blanket duplicate local suites while retaining meaningful regressions, domain proof and required hosted checks.

## Replacement after the exact-candidate public API failure

The first candidate failed one unchanged API fixture (4,017 passed / 1 failed). The sole correction returns the typed decision-specific gate mapping only for mapping declarations, restoring the public list-based payload shape without changing simulated gate meaning. The unchanged fixture then passed;95 focused capability/scenario/graph/all-case tests passed. All925 nodes and1,045 semantic edges remain equal to the first generated F projection after only generated IDs/relationship keys are normalized. The API fixture and adapter are unchanged.

The source-bound second allocation generated GRAPH-SAU-2026-09-12-4152824cfa67 (ENGINE-64e24689a59f), SHA-256 ab5890d7af3037e548827b17d498a19ebc221eca5df5941f55b18e3a978a1e60. All seven stock generation/integrity/reconstruction commands passed. Snapshot membership is734: the earlier F pair remains immutable history and the replacement adds two files. The first canonical output and failed candidate remain historical, not final acceptance. Final capture, exact-candidate suite, strict R1 and independent review follow.

The first freeze recorded an ordinary-index stat-cache change during the prohibited-file scanner (causal inference from timestamps); zero staged paths and HEAD-equivalent index content were verified. No byte restoration was attempted. GIT_OPTIONAL_LOCKS=0 is now set for every Git/check process. See the retained index disposition rather than claiming the bytes never changed. No branch commit or Aura operation has occurred.
