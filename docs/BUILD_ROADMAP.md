...
S12a is MERGED (PR #15, `cc85cbc`) with five honest unavailable-source attempts. S12b is MERGED (PR #17, `9a9d5c7`) with twelve COMPLETE DocumentRecords and five honest unavailable sources. S12c has implementation evidence on branch `slice/s12c-entity-resolution-bilingual-ids`: 38 builder-verified mentions produce five company IDs, two SITE_LOCALITY plant IDs, no line or licence-holder IDs, three reviewer-pending links and eight unresolved links in one write-once artifact under `data/entities/`; the artifact remains outside engine consumption until S13/S14. At T7 its single manifest run is authorized but not executed, and independent review/PR/CI/merge remain pending. Extraction metrics and graph work remain in later slices; no parent obligation is closed by test-double rows alone.
