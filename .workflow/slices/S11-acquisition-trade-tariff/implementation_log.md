...
**Scope.** Owner ruling 4 (`.autonomous-workflow/owner-decisions/s11-acquisition-trade-tariff-4.json`, SHA-256 `dc68d130a43bc7cb61abc5412de7f0f00c3c6149ad45e41fe61fd8a31d5f45d6`) superseded plan-5 on P5-F01 and authorized plan-6. Plan-6 delta only: DD-25/DD-26 rewrite of `tests/test_frozen_public_evidence_pins.py` — `FROZEN_TREE_OIDS`, `FrozenTreeCheckError`, `frozen_tree_problems`, eleven offline detector tests, production git assertions re-targeted to HEAD-relative tree pins; `BASE` and `_git_diff_quiet` removed; ADR-015 paragraph; control/slice records; plan-6 pointers.

**Supersession.** Plan-5 (`.autonomous-workflow/plans/s11-acquisition-trade-tariff/plan-5.json`, SHA-256 `de5f9e1a7ec56a1bfe159a77785b8dc55cfddafa6354e7bc3ac5915c73fccba2`) was reviewer-grok REJECT on P5-F01: verification[35] used `git ls-files --others` without `--exclude-standard` on the broad delta surface, so ignored `__pycache__` / `*.egg-info` trees recreated by compileall/pytest/make ci made the gate exit 1 on a correct candidate. Plan-6 corrects only verification[35]; the four-root strict no-exclude rejection in verification[4] and `frozen_tree_problems` is unchanged.

**CI-F-01.** PR #14 head `acc00092` run 33828693267: three Python jobs failed `tests/test_frozen_public_evidence_pins.py` with `fatal: bad object a610b49…` (`git diff --quiet a610b49 -- …` on depth-1 checkout). Frozen bytes unchanged. CI result JSON `.autonomous-workflow/evidence/s11-acquisition-trade-tariff/ci/pr-14-acc00092d32b.json` SHA-256 `f32fdb168aeeab0bc961e309d1d46b4fd35f9541ea533dc47a424e5dddc63a38`; sibling log `.autonomous-workflow/evidence/s11-acquisition-trade-tariff/ci/pr-14-acc00092d32b.log` SHA-256 `3f4e9abbd502097f37ff8334b465e7c55aefab35fd7c1c539214b505c15c7652`; failure log `.autonomous-workflow/evidence/s11-acquisition-trade-tariff/ci-failure-1/run-33828693267-failed.log` SHA-256 `0ea19e464aa76407c1cd63dc81a09684752aebc01a6b4f6d4547ebbfa8b6ab67`.

**Immutability.** No acquire-* command run; no `build_manifests.py` run. No byte under `data/**`, `config/**`, `docs/core/**`, `docs/authority/**`, `browser_tests/**`, `src/**`, `scripts/**`, `.github/**` or `Makefile` changed.

**TDD evidence (`.autonomous-workflow/evidence/s11-acquisition-trade-tariff/6/`).** `red-0-shallow-clone-inherited-module`: verification[36] on inherited module — 2 failed (base-commit git diff), 4 passed. `red-1-detectors-before-checker`: eleven detector tests against stub without checker — 12 failed (NameError). `green-1-module-contract`: verification[37] MODULE CONTRACT PASS (17 tests). `green-2-shallow-clone-candidate-module`: verification[36] on candidate module in depth-1 file:// clone with `a610b49` absent — 17 passed.
