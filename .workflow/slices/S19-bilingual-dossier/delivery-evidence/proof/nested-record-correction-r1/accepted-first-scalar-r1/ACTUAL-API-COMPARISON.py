...
O = E / 's19/nested-record-correction-r1/accepted-first-scalar-r1'
...
post = {'source': json.loads((O / 'SOURCE.json').read_text()),
        'indexes': json.loads((E / 's19/canonical-operation-r1/INPUT-BINDING.json').read_text())['indexes']}
cases = sorted(public_cases())
assert len(cases) == 11
old_css = (E / 's19/final-print-correction-r1/preimages/src/ior_mvp/static/css/dossier.css').read_bytes()
new_css = (O / 'source/src/ior_mvp/static/css/dossier.css').read_bytes()
assert hashlib.sha256(old_css).hexdigest() == 'fb10139b6c278f08be68d26534633b6cb966de91533156319a9b92d7b47ef818'
assert hashlib.sha256(new_css).hexdigest() == json.loads((O / 'SOURCE.json').read_text())['src/ior_mvp/static/css/dossier.css']
...
result = {
    ...
    'source_ledger_sha256': sha(O / 'SOURCE.json'),
